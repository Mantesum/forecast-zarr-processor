...
def test_plan_is_deterministic_and_includes_derived_wind(tmp_path: Path) -> None:
    ...
    assert (
        next(item for item in first.variables if item.name == "wind_speed_10m").group == "derived"
    )
